...
export default function ChargeFormArray({FolioID, TransactionID, handleModal}: any){
...
const {
    register, reset, resetField, handleSubmit, control, formState: {errors},
}=useForm({
    defaultValues:{
        charge:[{
        GroupID: null,
...
const onSubmit = async(data:any) => {
    ...
    for(const index in data.charge){
        ...
        data.charge[index].Amount=data.charge[index].Amount*Number(data.charge[index].Quantity);
...
<Stack direction='row' spacing={2}  alignItems='center' mb={2}>
                ...
                <TextField disabled value={ moment(workingDate).format("YYYY-MM-DD")} fullWidth/>
                ...
                <DateTimePicker disabled={enableDate} value={setedDate} onChange={(newValue:any)=> setSetedDate(newValue)}   renderInput={(params) => ( <TextField fullWidth {...params}/> )}/>
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    {fields.map((field,index)=>(
                        <>
                            ...
                            <FolioCharge id={index} register={register}
                            remove={remove} FolioID={FolioID} TransactionID={TransactionID}
                    ...
                    <Button onClick={()=>{
                        append({GroupID: null, ItemID: null, Amount: null, Quantity: 1, Description: ' '})}}>
                        Add charge
                    </Button>
                </Stack>
