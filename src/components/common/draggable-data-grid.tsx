...
const DraggableTableRow = ({ children, id, excludeFromDragDrop, customizedColumns, ...props }: any) => {
  const {
    attributes,
    listeners,
    setNodeRef,
...
const DraggableDataGrid = ({
  ...
  enableDragDrop = true,
  onRowReorder,
  excludeFromDragDrop = ["TextPhone", "RowHeaderStatus"], // Columns to exclude from drag and drop
}: any) => {
  const intl = useIntl();
  const theme = useTheme();
  ...
  // Drag state for visual effects
  const [isDragging, setIsDragging] = useState(false);
  const [draggedRoomType, setDraggedRoomType] = useState<string | null>(null);
  ...
  const handleDragStart = (event: any) => {
    const { active } = event;
    const draggedIndex = data.findIndex((item: any) => (id ? item[id] : item.id) === active.id);
    const draggedItem = data[draggedIndex];
    
    setIsDragging(true);
    if (draggedItem?.RoomTypeName) {
      setDraggedRoomType(draggedItem.RoomTypeName);
    }
  };
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (active.id !== over?.id) {
      const oldIndex = data.findIndex((item: any) => (id ? item[id] : item.id) === active.id);
      const newIndex = data.findIndex((item: any) => (id ? item[id] : item.id) === over?.id);
      
      const draggedItem = data[oldIndex];
      const targetItem = data[newIndex];
      
      // Check if both items belong to the same room type (if RoomTypeName exists)
      if (draggedItem.RoomTypeName && targetItem.RoomTypeName) {
        if (draggedItem.RoomTypeName !== targetItem.RoomTypeName) {
          // Don't allow moving between different room types
          return;
        }
      }
      ...
      if (onRowReorder) {
        onRowReorder(newData, oldIndex, newIndex);
      }
    }
  };
              ...
              <DataGrid
                ...
                sx={{
                  ...
                  '& .MuiDataGrid-row': {
                    ...
                    '&.Mui-selected': {
                      backgroundColor: alpha("#884ce4", 0.08),
                    }
                  },
                  '& .MuiDataGrid-cell': {
              ...
              >
                ...
                >
                  <Table size="small" stickyHeader sx={{
                    ...
                    <TableBody>
                      <SortableContext
                        items={data.map((item: any) => id ? item[id] : item.id)}
                      ...
                      >
                        {data.map(
                          (
                            ...
                            const rowId = id ? element[id] : element.id;
                            const isOtherRoomType = isDragging && draggedRoomType && element.RoomTypeName && element.RoomTypeName !== draggedRoomType;
                            
                            return enableDragDrop ? (
                              <DraggableTableRow
                                key={rowId}
                                id={rowId}
                                excludeFromDragDrop={excludeFromDragDrop}
                              ...
                              >
                                {customizedColumns.map(
                                  (
                                    ...
                                    const isExcludedColumn = excludeFromDragDrop.includes(column.key);
                                    return (
                                    <TableCell
                                      key={index}
                                      className={
