...
const AmendStayForm = ({
  ...
  const setRange = (dateStart: Date, dateEnd: Date) => {
    ...
    if (isNaN(dateStart.getTime()) || isNaN(dateEnd.getTime())) {
      console.warn('Invalid dates passed to setRange:', { dateStart, dateEnd });
      return;
    }
  ...
  useEffect(() => {
    // Validate and create date objects with fallback to current date
    var dateStart = new Date(transactionInfo.ArrivalDate);
    var dateEnd = new Date(transactionInfo.DepartureDate);

    // Check if dates are valid, if not use current date as fallback
    if (isNaN(dateStart.getTime())) {
      console.warn('Invalid ArrivalDate:', transactionInfo.ArrivalDate);
      dateStart = new Date();
    }
    if (isNaN(dateEnd.getTime())) {
      console.warn('Invalid DepartureDate:', transactionInfo.DepartureDate);
      dateEnd = new Date();
      dateEnd.setDate(dateEnd.getDate() + 1); // Set departure to next day
    }
  ...
  const onArrivalTimeChange = (evt: any) => {
    // Validate baseStay dates before processing
    if (!baseStay.dateStart || isNaN(baseStay.dateStart.getTime()) ||
      !baseStay.dateEnd || isNaN(baseStay.dateEnd.getTime())) {
      console.warn('Invalid baseStay dates in onArrivalTimeChange:', baseStay);
      return;
    }
  ...
  const onDepartureTimeChange = (evt: any) => {
    ...
    if (!baseStay.dateStart || isNaN(baseStay.dateStart.getTime()) ||
      !baseStay.dateEnd || isNaN(baseStay.dateEnd.getTime())) {
      console.warn('Invalid baseStay dates in onDepartureTimeChange:', baseStay);
      return;
    }
