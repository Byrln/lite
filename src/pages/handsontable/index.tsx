    ...
    const TimelineTable = ({props, workingDate}: any) => {
        ...
        const {data: items, mutate: mutateItems, error: itemsError} = FrontOfficeSWR(workingDate, dayCount);
        ...
        useEffect(() => {
            ...
            for (let i=0; i < parseInt(dayCount); i ++){
                ...
                headers.push({label : date_temp.getDate() + "<br>" + (date_temp.getMonth() + 1).toString() + " сар<br>" + week, colspan: 2});
            }
        ...
        function clickCell(event, coords){
            ...
            let key = Object.keys(coldict).filter(function(key) {return coldict[key] === col})[0];
            console.log("Column date: ", key)
            console.log("Records: ", records[coords.row].id, records[coords.row].room)
            // @ts-ignore
            console.log("Finding order by coordinate: ", orderCoords[coords.row + '_' + (coords.col - 1)])
            ...
            for (i in roomTypes) {
                ...
                for (j in rooms) {
                    if (rooms[j].RoomTypeID == roomTypes[i].RoomTypeID) {
                        // @ts-ignore
                        temp_rec = {
                            id: "" + roomTypes[i].RoomTypeID + "_" + rooms[j].RoomID,
                            room: rooms[j].RoomNo,
                        }
                        for (let l=0; l < parseInt(dayCount) * 2; l ++){
                            // @ts-ignore
                ...
                for (let j in records){
                    ...
                    if (groupKey == records[j]['id']) {
                        ...
                        if (start_index !== null && start_index !== undefined){
                            ...
                            // @ts-ignore
                            mergeCell['rowspan'] = 1
                            if (begin_start_index == null || begin_start_index == undefined){
                                // @ts-ignore
                                mergeCell['colspan'] = day_count > 0 ? day_count - 1 : 1;
                            }else {
                                // @ts-ignore
                            ...
                            // @ts-ignore
                            cellStyle['className'] = 'color-' + items[i]['StatusColor'].toUpperCase();
